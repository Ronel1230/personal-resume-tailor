...
// TEMPLATE 9: DOUBLE RULE - Centered header with burgundy double lines
export async function renderTemplate9(context: TemplateContext): Promise<Uint8Array> {
  ...
  const BURGUNDY = rgb(0.45, 0.15, 0.2);
  ...
  if (name) {
    ...
    page.drawText(name, { x: (PAGE_WIDTH - nameWidth) / 2, y, size: NAME_SIZE, font: fontBold, color: CHARCOAL });
    y -= NAME_SIZE + 8;
  }
  ...
  if (contactParts.length > 0) {
    ...
    page.drawText(contactText, { x: (PAGE_WIDTH - contactWidth) / 2, y, size: CONTACT_SIZE, font, color: MEDIUM_GRAY });
    y -= CONTACT_SIZE + 10;
  }

  const lineWidth = 200;
  const lineX = (PAGE_WIDTH - lineWidth) / 2;
  page.drawLine({ start: { x: lineX, y }, end: { x: lineX + lineWidth, y }, thickness: 1.5, color: BURGUNDY });
  y -= 5;
  page.drawLine({ start: { x: lineX, y }, end: { x: lineX + lineWidth, y }, thickness: 0.5, color: BURGUNDY });
  y -= 20;
  ...
  for (let i = 0; i < bodyLines.length; i++) {
    ...
    if (line.endsWith(':')) {
      ...
      page.drawText(sectionName, { x: (PAGE_WIDTH - sectionWidth) / 2, y, size: SECTION_SIZE, font: fontBold, color: BURGUNDY });
