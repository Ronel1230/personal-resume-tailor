...
// TEMPLATE 8: BOXED SECTIONS - Light gray boxes behind section headers
export async function renderTemplate8(context: TemplateContext): Promise<Uint8Array> {
  ...
  const BOX_FILL = rgb(0.93, 0.93, 0.94);
  ...
  for (let i = 0; i < bodyLines.length; i++) {
    ...
    if (line.endsWith(':')) {
      ...
      page.drawRectangle({
        ...
        y: y - 4,
        width: CONTENT_WIDTH,
        height: SECTION_BOX_HEIGHT,
        color: BOX_FILL,
      });
      page.drawText(sectionName, { x: MARGIN_LEFT + 6, y, size: SECTION_SIZE, font: fontBold, color: CHARCOAL });
      y -= SECTION_BOX_HEIGHT + SPACING.AFTER_SECTION_HEADER - 4;
      isFirstJob = true;
