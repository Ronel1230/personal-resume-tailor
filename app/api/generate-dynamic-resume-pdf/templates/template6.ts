...
// TEMPLATE 6: TEAL BANNER - Colored header band with white name
export async function renderTemplate6(context: TemplateContext): Promise<Uint8Array> {
  ...
  drawHeaderBand(page);
  let y = PAGE_HEIGHT - HEADER_BAND_HEIGHT - 14;
  ...
  for (let i = 0; i < bodyLines.length; i++) {
    ...
    if (line.endsWith(':')) {
      ...
      page.drawText(sectionName, { x: MARGIN_LEFT, y, size: SECTION_SIZE, font: fontBold, color: TEAL });
      y -= 4;
      page.drawLine({
