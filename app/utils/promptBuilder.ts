...
2. Preserve all original company names, job titles, and periods/dates in the Professional Experience section.
  ...
  2b. NO NEW COMPANIES OR INDUSTRIES: Never invent or add new companies or roles (including for industry alignment). If no existing company clearly matches the JD's industry, still keep all original companies and their implied industries; only adjust wording and responsibilities to highlight transferable, JD-relevant skills where realistic.
3. In each Experience/job entry, produce 8-10 bullets (one sentence per bullet), each a concise storytelling sentence (challenge - action - result). This is a hard requirement: NEVER fewer than 8 bullets per role. The longest company should have 10 bullets, and the others should have 8-10 bullets according to company period length. Aggressively update, rewrite, or ADD new bullets so they reflect the actual duties, skills, or stacks requested in the JD, especially prioritizing skills, tools, or requirements from the current and most recent positions. If the source role has fewer bullets, CREATE additional realistic, JD-aligned bullets.
  3a. FORMAT: Each experience bullet MUST be on its own line. Each line under a job MUST start with a bullet character (- followed by a space) then exactly one sentence. Do NOT put multiple sentences or multiple bullets on one line. Example: "- Built X." on one line, "- Led Y." on the next line.
...
- Use ASCII punctuation only in the resume output: plain hyphens (-), straight quotes (' and "), commas, semicolons, and periods. Do NOT use Unicode arrows, em/en dashes, curly quotes, ellipsis characters, or other special symbols.
